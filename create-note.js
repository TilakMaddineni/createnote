...
window.hoveringNote.load = function(window, jQuery, window_md, d) {
	$ = jQuery;
	md = window_md;
	debug = d;
	window.hoveringNote.main = main;
};
...
var main = function() {
	var tpls = {};
	var app = new window.hoveringNoteApp($, md, debug);
	var width = 320;
	var width_key = '__create-note-width'
	...
	var receive_message = function(event) {
		...
		if ( event.data.type === 'loaded' ) {
			...
			$iframe.addClass('__create-note_opened');
			$iframe[0].scrolling = 'auto';
			$iframe.attr({scrolling: 'auto'});
		} else if ( event.data.type === 'close' ) {
			$iframe.removeClass('__create-note_opened');
			$iframe[0].scrolling = 'no';
			$iframe.attr({scrolling: 'no'});
		} else if ( event.data.type === 'toWindow' ) {
			if ( hoveringNote.notes[event.data.id] ) {
				hoveringNote.notes[event.data.id].$el.remove();
				delete hoveringNote.notes[event.data.id];
			}

			event.data.note.id = event.data.id;
			var note = new window.hoveringNote.Note(event.data.note, tpls['note-tpl'], tpls['note-form-tpl'], save_note, iframe.contentWindow);
			note.$el.appendTo(document.body);
			hoveringNote.notes[event.data.id] = note;
		} else if ( event.data.type === 'toList' && hoveringNote.notes[event.data.id] ) {
			hoveringNote.notes[event.data.id].$el.remove();
			delete hoveringNote.notes[event.data.id];
		} else if ( event.data.type === 'delete' && hoveringNote.notes[event.data.id] ) {
			hoveringNote.notes[event.data.id].$el.remove();
			delete hoveringNote.notes[event.data.id];
		}
	};
	...
	// Add styling
	var $css = $('<link rel="stylesheet" id="__create-note-css"/>');
	$css.attr({href: '//{{ site.domain }}/css/create-note.css'});
	$css.appendTo(document.head);

	// Add iframe
	var $iframe = $('<iframe id="__create-note" scrolling="no" style="display: none; opacity: 0; width: ' + width + 'px"></iframe>');
	var iframe = $iframe[0];
	$iframe.attr({src: '//{{ site.domain }}/greasemonkey/create-note/create-note-iframe.html#' + document.location.href});
	...
	var note_end_move = function(e) {
		...
		$body
			.removeClass('__create-note--moving')
	...
	var note_end_resize = function() {
		var note = hoveringNote.get_node(resizing);
	...
	// Events
	$(document)
		.delegate('.__create-note--note', 'focusin', function(e) {
			$(this).addClass('__create-note--focused');
		})
		.delegate('.__create-note--note', 'focusout', function(e) {
			$(this).removeClass('__create-note--focused');
		})
		.delegate('[data-type="mover"]', 'mousedown', function(e) {
			...
			moving_node = hoveringNote.get_node(this);
		...
		.delegate('[data-type="resize"]', 'mousedown', function(e) {
			...
			resizing_node = hoveringNote.get_node(this);
		...
		.delegate('[data-type="to-list"]', 'click', function() {
			hoveringNote.get_node(this).toList();
			hoveringNote.get_node(this).$el.remove();
		})
		;
};

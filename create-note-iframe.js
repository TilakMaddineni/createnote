...
var debug = window.hoveringNoteDebug;
...
var blacklist_name = '__create-note-blacklist';
var blacklist;
...
var is_url_blacklisted = function(url) {
	for ( var key in blacklist ) {
		if ( blacklist.hasOwnProperty(key) ) {
			...
			if ( b.m && hoveringNote.fauxMatch(b.m, url) ) {
				return true;
			}
		}
	}
...
var receive_message = function(event) {
	...
	if ( event.data.type === 'save_note' ) {
		var onote = hoveringNote.notes[event.data.id];
		...
		for ( var key in hoveringNote.notes ) {
			if ( hoveringNote.notes.hasOwnProperty(key) ) {
				var note = hoveringNote.notes[key];
		...
		hoveringNote.notes[event.data.id].$el.removeClass('to-window');
...
$document
	.delegate('[data-type="to-window"]', 'click', function() {
		hoveringNote.get_node(this).toWindow();
		hoveringNote.get_node(this).$el.addClass('to-window');
	})
	.delegate('[data-type="to-list"]', 'click', function() {
		hoveringNote.get_node(this).toList();
		hoveringNote.get_node(this).$el.removeClass('to-window');
	})
...
var blacklist_data_name = '__create-note-blacklist-id';
...
var gd_auth_name = '__create-note-gdauth';
...
var notes_name = '__create-note-notes';
var notes = {};
var gdsaver_notes = new GDSaver(notes_name);

var matches = 0;
var add_note = function(values) {
	var note = new hoveringNote.Note(values, note_tpl, note_form_tpl, save_note, window.parent);

	hoveringNote.notes[note.id] = note;

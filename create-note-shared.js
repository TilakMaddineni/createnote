...
Note.prototype.init = function(params) {
	...
	/* Shadow DOM
	// div.innerHTML = html;
	var css = '<link rel="stylesheet" href="//{{ site.domain }}/css/create-note.css" id="__create-note-css"/>';
	...
	*/
	...
	this.$el.data('__create-note-object', this);
};
...
Note.prototype.edit = function() {
	...
	this.editing = true;
	this.$el
		.addClass('__create-note--editing')
...
Note.prototype.save = function(params, url, skip_save_fn) {
	...
	if ( ignore_next_update === this.id ) {
		...
		this.$el
			.removeClass('__create-note--editing')
			.find('.__create-note--form')
				.remove().end()
			.find('.__create-note--mover').html(this.title).end()
			.find('.__create-note--match').html(this.match).end()
			.find('.__create-note--body').html(md(this.body)).end()
			.css({
				width: this.width,
				height: this.height,
				top: this.y,
	...
	if ( this.absolute ) {
		this.$el.addClass('__create-note--absoluted');
	} else {
		this.$el.removeClass('__create-note--absoluted');
	}
};
...
var hoveringNote = window.hoveringNote = {};

hoveringNote.get_node = function(el) {
	return $(el).closest('.__create-note--note').data('__create-note-object');
};
...
hoveringNote.fauxMatch = function(pattern, str) {
	pattern = pattern
		.replace(/([\.\\\+\?\[\^\]\$\(\)\{\}\=\!\>\|\:\-])/g, '\\$1')
		.replace(/\*/g, '.*')
		.replace(/\/\.\*$/, '(\\/.*)?')
...
hoveringNote.compile = function(html) {
...
window.hoveringNote.Note = Note;

var hoveringNoteApp = window.hoveringNoteApp = function(jQuery, window_md, d) {
	if ( d ) {
		debug = true;
	}
...
// Template
hoveringNoteApp.prototype.tpl = function(source) {
	var template = hoveringNote.compile(source);
	return template;
};

hoveringNoteApp.prototype.attach = function() {
	$(document)
		.delegate('.__create-note--body input[type="checkbox"]', 'change', function() {
			var checkbox = this;
			var $this = $(this);
			var checked = $this.prop('checked');
			var $mnbody = $this.closest('.__create-note--body');
			...
			var note = hoveringNote.get_node(this);
			var s = note.body;
		...
		.delegate('[data-type="min"]', 'click', function() {
			hoveringNote.get_node(this).minimize();
		})
		.delegate('[data-type="restore"]', 'click', function() {
			hoveringNote.get_node(this).restore();
		})
		.delegate('.__create-note--form textarea', 'input drop paste cut delete click', function() {
			autosize(this);
		})
		.delegate('[data-type="edit"]', 'click', function() {
			var node = hoveringNote.get_node(this);
			node.edit();
			autosize(node.$el.find('textarea')[0]);
		})
		.delegate('[data-type="save"]', 'click', function(e) {
			e.preventDefault();
			hoveringNote.get_node(this).save($(this).closest('form').serializeArray());
		})
		.delegate('[data-type="cancel"]', 'click', function(e) {
			e.preventDefault();
			hoveringNote.get_node(this).cancel();
		})
		.delegate('[data-type="delete"]', 'click', function(e) {
			...
			if ( note.del() ) {
				note.$el.remove();
				delete hoveringNote.notes[note.id];
			}
		})
		;
};
